...
**Docker:** set the Jellyfin image in Compose to `ghcr.io/nintwentydo/jellyfin-postgres:1.1.3.0-jf13-custom`, then run:
...
Check the logs and saved state after startup. The image upgrades server and plugin together. The stable `latest` image and dashboard catalogue do not supply this custom pair.
